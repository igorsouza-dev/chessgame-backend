...
describe('Routes', () => {
  ...
  test('it should increment the move number when a move is made', async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body).toHaveProperty('board');
    expect(response.body.move_number).toStrictEqual(1);
    expect(response.body.turn_player).toStrictEqual('W');
    ...
    expect(response.status).toBe(200);
    expect(response.body).toHaveProperty('board');
    expect(response.body.move_number).toStrictEqual(2);
    expect(response.body.turn_player).toStrictEqual('B');
  });
